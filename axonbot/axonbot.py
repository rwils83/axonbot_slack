...
@decorators.required_settings(['AX_URL', 'AX_KEY', 'AX_SECRET', 'AX_USER_FIELDS', 'AX_DEVICE_FIELDS'])
class AxonBot(base.MachineBasePlugin):
    """Axonius Slack Bot."""

    def init(self):
        """Pass."""
        ax_log_level = self.settings.get("AX_LOG_LEVEL", logging.INFO)
        logging.getLogger("axonius_api_client").setLevel(ax_log_level)

        self._http_client = axonius_api_client.http.HttpClient(url=self.settings["AX_URL"])
        self._http_client.session.proxies = {}
        self._http_client.session.proxies["http"] = self.settings.get("AX_HTTP_PROXY", "")
        self._http_client.session.proxies["https"] = self.settings.get("AX_HTTPS_PROXY", "")
        self._auth_method = axonius_api_client.auth.AuthKey(
            http_client=self._http_client, key=self.settings["AX_KEY"], secret=self.settings["AX_SECRET"]
        )
        self._auth_method.login()

        self._api_users = axonius_api_client.api.Users(auth=self._auth_method)
        self._api_users._type = "users"
        self._api_users._get_fields = self.settings["AX_USER_FIELDS"]

        self._api_devices = axonius_api_client.api.Devices(auth=self._auth_method)
        self._api_devices._type = "devices"
        self._api_devices._get_fields = self.settings["AX_DEVICE_FIELDS"]
    ...
    def fields_user(self, msg):
        ...
        send_text = self.build_get_fields_lines(api_type=self._api_users)
        msg.reply(send_text, in_thread=True)

    @decorators.respond_to(regex=r'^fields user add (?P<adapter>\S+) (?P<field>\S+)')
    def fields_user_add(self, msg, adapter, field):
        """fields user add [adapter] [field]: Add an adapters field for user responses."""
        self.field_add(msg=msg, api_type=self._api_users, adapter=adapter, field=field)

    @decorators.respond_to(regex=r'^fields user delete (?P<adapter>\S+) (?P<field>\S+)')
    def fields_user_delete(self, msg, adapter, field):
        """fields user delete [adapter] [field]: Delete an adapters field for user responses."""
        self.field_del(msg=msg, api_type=self._api_users, adapter=adapter, field=field)

    @decorators.respond_to(regex=r'^fields device$')
    def fields_device(self, msg):
        """fields device: Show the fields that will be returned in responses."""
        send_text = self.build_get_fields_lines(api_type=self._api_devices)
        msg.reply(send_text, in_thread=True)

    @decorators.respond_to(regex=r'^fields device add (?P<adapter>\S+) (?P<field>\S+)')
    def fields_device_add(self, msg, adapter, field):
        """fields device add [adapter] [field]: Add an adapters field for device responses."""
        self.field_add(msg=msg, api_type=self._api_devices, adapter=adapter, field=field)

    @decorators.respond_to(regex=r'^fields device delete (?P<adapter>\S+) (?P<field>\S+)')
    def fields_device_delete(self, msg, adapter, field):
        """fields device delete [adapter] [field]: Delete an adapters field for device responses."""
        self.field_del(msg=msg, api_type=self._api_devices, adapter=adapter, field=field)
    ...
    def user_by_query(self, msg, value):
        ...
        self.fetch_query(api_type=self._api_users, query=value, msg=msg)

    @decorators.respond_to(regex=r'^get user username (?P<value>\S+)')
    def user_by_username(self, msg, value):
        """get user username [value]: Get users by username (prefix value with *re=* to use regex)"""
        self.fetch_by(
    ...
    def user_by_email(self, msg, value):
        """get user email [value]: Get users by email (prefix value with *re=* to use regex)"""
        self.fetch_by(
            api_type=self._api_users, value_name="email", value=value, field="mail", field_adapter="generic", msg=msg
        )
    ...
    def device_by_query(self, msg, value):
        ...
        self.fetch_query(api_type=self._api_devices, query=value, msg=msg)
    ...
    def get_by_saved_query_users(self, msg, value):
        ...
        self.get_by_saved_query(msg=msg, api_type=self._api_users, value=value)
    ...
    def get_by_saved_query_devices(self, msg, value):
        ...
        self.get_by_saved_query(msg=msg, api_type=self._api_devices, value=value)
    ...
    def get_device_saved_queries(self, msg):
        ...
        self.get_saved_queries(msg=msg, api_type=self._api_devices)
    ...
    def get_user_aved_queries(self, msg):
        ...
        self.get_saved_queries(msg=msg, api_type=self._api_users)

    @decorators.respond_to(regex=r'^get device hostname (?P<value>\S+)')
    def device_by_hostname(self, msg, value):
        """get device hostname [value]: Get devices by hostname (prefix value with *re=* to use regex)"""
        self.fetch_by(
    ...
    def device_by_mac(self, msg, value):
        """get device mac [value]: Get devices by MAC address (prefix value with *re=* to use regex)"""
        self.fetch_by(
            api_type=self._api_devices,
    ...
    def device_by_ip(self, msg, value):
        """get device ip [value]: Get devices by ip (prefix value with *re=* to use regex)"""
        self.fetch_by(
            api_type=self._api_devices,
    ...
    def handle_labels(self, event, msg):
        if msg.thread_ts not in TTL_CACHE:
            send_text = "No objects requested or objects have expired, get an object first!"
            msg.reply(send_text, in_thread=True)
            return
        ...
        if not label_sub_cmd:
            ...
            send_text = send_text.format(cmds=", ".join([i["cmd"] for i in LABELS_SUB_CMDS]))
            msg.reply(send_text, in_thread=True)
            return
    ...
    def fetch_query(self, api_type, query, msg):
        try:
            rows = list(api_type.get(query=query, row_count_min=1, **api_type._get_fields))
            ...
            self.upload_file_reply(msg=msg, filename=filename, content=json_rows)

    def fetch_by(self, api_type, value_name, value, field, field_adapter, msg):
        kwargs = {}
        ...
        try:
            ...
            send_text = send_text.format(api_type=api_type._type, value=value, value_name=value_name)
            msg.reply(send_text, in_thread=True)
        except Exception as exc:
            send_text = "Error fetching {api_type} matching {value_name} {value!r}: {exc}"
            send_text = send_text.format(api_type=api_type._type, value=value, value_name=value_name, exc=exc)
            msg.reply(send_text, in_thread=True)
        else:
            TTL_CACHE[msg.thread_ts] = {"api_type": api_type, "rows": rows}
            json_rows = json.dumps(rows, indent=2)
    ...
    def upload_file_reply(self, msg, filename, content, in_thread=True, initial_comment=None, filetype=None):
        kwargs = {}
        ...
        send_text = send_text.format(filename=filename, channel=msg.channel.id, thread=kwargs.get("thread_ts", None))
        text.announce(send_text)
        return response

    def build_get_fields_lines(self, api_type):
    ...
    def find_field(self, api_type, adapter, field, msg):
        try:
            known_fields = api_type.get_fields()
            if adapter != "generic":
                known_adapters = list(known_fields["specific"].keys())
                adapter = axonius_api_client.api.utils.find_adapter(name=adapter, known_names=known_adapters)
            ...
            known_names = "\n\t" + "\n\t".join(exc.known_names)
            send_text = (
                "No such field named {field!r} available in adapter {adapter!r}, valid field names: {known_names}"
            )
            send_text = send_text.format(field=field, adapter=adapter, known_names=known_names)
            msg.reply(send_text, in_thread=True)
        except Exception as exc:
    ...
    def field_add(self, msg, api_type, adapter, field):
        adapter, field = self.find_field(api_type=api_type, adapter=adapter, field=field, msg=msg)
        ...
        if field and adapter:
            ...
            send_text = send_text.format(field=field, adapter=adapter, api_type=api_type._type)
            msg.reply(send_text, in_thread=True)

            send_text = self.build_get_fields_lines(api_type=api_type)
            msg.reply(send_text, in_thread=True)

    def field_del(self, msg, api_type, adapter, field):
        adapter, field = self.find_field(api_type=api_type, adapter=adapter, field=field, msg=msg)

        if field and adapter:
            if adapter not in api_type._get_fields:
                send_text = "Adapter {adapter} is not in the current {api_type} fields!"
                send_text = send_text.format(adapter=adapter, field=field, api_type=api_type._type)
                msg.reply(send_text, in_thread=True)
            ...
            if field not in api_type._get_fields[adapter]:
                send_text = "Field {field} for adapter {adapter} is not in the current {api_type} fields!"
                send_text = send_text.format(adapter=adapter, field=field, api_type=api_type._type)
                msg.reply(send_text, in_thread=True)
    ...
    def get_saved_queries(self, msg, api_type):
    ...
    def get_by_saved_query(self, api_type, msg, value):
        try:
            sq = api_type.get_saved_query_by_name(name=value, regex=False, only1=True)
            rows = list(api_type.get(query=sq["view"]["query"]["filter"], manual_fields=sq["view"]["fields"]))
